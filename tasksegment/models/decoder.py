...
class IrisMaskDecoder2D(nn.Module):
    ...
    def forward(self, query_feats, task_tokens):
        ...
        if self.dense_guidance is not None and text_tokens is not None and text_tokens.shape[1] > 0:
            ...
            # dense_guidance_lowres is logits because the guidance branch is trained
            # with BCEWithLogitsLoss. Convert to probability before feature modulation.
            dense_guidance_prob = torch.sigmoid(dense_guidance_lowres)
            dense_guidance_for_mod = dense_guidance_prob.clamp(0.0, self.guidance_clamp_max)
