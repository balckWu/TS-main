...
PROMPT_BANK: Dict[str, Dict[str, List[str]]] = {
    ...
    "OTU": {
        ...
        "boundary": [
            "irregular tumor contour",
            "partially blurred margin",
            "closed ovarian lesion boundary",
            "mass capsule boundary",
        ],
        "exclusion": [
            "normal ovarian tissue",
            "background pelvic tissue",
            "anechoic fluid region",
            "ultrasound speckle artifact",
        ],
        "task": [
            "segment the ovarian tumor region",
            "separate the ovarian mass from surrounding tissue",
            "focus on the true tumor pixels",
        ],
    },
    ...
    "prostate": {
        "anatomy": [
            ...
            "focal prostate abnormality",
        ],
        "appearance": [
            "focal hypoechoic region",
            "heterogeneous prostate echotexture",
            "low contrast gland boundary",
            "compact target region",
        ],
        "boundary": [
            "poorly defined prostate margin",
            "subtle lesion boundary",
            "closed gland contour",
            "ambiguous ultrasound boundary",
        ],
        "exclusion": [
            "normal prostate tissue",
            "background pelvic structure",
            "ultrasound speckle noise",
            "non target region",
        ],
        "task": [
            "segment the prostate target region",
            "separate the prostate foreground from surrounding tissue",
            "focus on prostate lesion or gland pixels",
        ],
    },
...
def build_all(model_path: str, output_dir: str):
    all_datasets = list(PROMPT_BANK.keys())
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="all",
        help="Dataset name: thyroid / TN3K / BUSI_WHU / BUS-BRA / OTU / prostate / all",
    )
