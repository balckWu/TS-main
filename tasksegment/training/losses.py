...
def boundary_weighted_ce_loss(logits: torch.Tensor, target: torch.Tensor, weight_map: torch.Tensor) -> torch.Tensor:
    ...
    if weight_map.shape[1] == 1:
        pixel_weight = weight_map[:, 0]
    elif weight_map.shape[1] == target_onehot.shape[1]:
        # If a class-channel weight map is provided, use the foreground/background
        # weights after summing CE over classes.
        pixel_weight = weight_map.mean(dim=1)
    else:
        raise ValueError("weight_map channel dimension error")

    per_pixel_ce = (-target_onehot * F.log_softmax(logits, dim=1)).sum(dim=1)
    return (per_pixel_ce * pixel_weight).mean()
