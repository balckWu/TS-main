        ...
        for organ_name, root_path in data_roots.items():
            ...
            for split_name in sub_dirs:
                ...
                for img_name in img_files:
                    ...
                    # ========================================================
                    # 💡 核心修改区：智能探寻对应掩码，解绑图像后缀与掩码后缀
                    # ========================================================
                    # 获取没有后缀的文件名主体 (例如 "001" 来自 "001.jpg")
                    base_name = os.path.splitext(img_name)[0]
                    
                    # 设定掩码的搜索优先级：强烈优先寻找纯正的 .png，其次是原图同名后缀
                    possible_mask_names = [
                        base_name + ".png", 
                        base_name + ".PNG",
                        img_name  # 退回兜底：如果就是原名的掩码
                    ]
                    
                    mask_path = None
                    for m_name in possible_mask_names:
                        temp_path = os.path.join(mask_dir, m_name)
